...
const PhotographyPage = () => {
  ...
  const subcategories = ['Tutorials', 'Gear'];
  ...
  const loadArticles = async () => {
    try {
      const params = { category: 'Photography', status: 'published', limit: 50 };
  ...
  return (
    <div data-testid="photography-page" className="bg-white">
      <Navbar />
      <div className="max-w-7xl mx-auto pt-32 pb-16 px-4 sm:px-6 md:px-8 lg:px-12">
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-4">
            <Camera size={32} className="text-[#0284C7]" weight="bold" />
            <h1 className="text-5xl font-black text-[#0A0F1C] tracking-tighter">Photography</h1>
          </div>
          <p className="text-lg text-[#475569]">Master underwater photography with expert tutorials and gear recommendations.</p>
        </div>
        <div className="flex gap-3 mb-12">
          <Link to="/photography" data-testid="photography-all-tab" className={`px-4 py-2 text-sm font-medium rounded-none border-b-2 transition-colors ${!subcategory ? 'border-[#0284C7] text-[#0284C7]' : 'border-transparent text-[#475569] hover:text-[#0284C7]'}`}>All Photography</Link>
          {subcategories.map((sub) => (
            <Link key={sub} to={`/photography/${sub.toLowerCase()}`} data-testid={`photography-${sub.toLowerCase()}-tab`} className={`px-4 py-2 text-sm font-medium rounded-none border-b-2 transition-colors ${subcategory === sub.toLowerCase() ? 'border-[#0284C7] text-[#0284C7]' : 'border-transparent text-[#475569] hover:text-[#0284C7]'}`}>{sub}</Link>
          ))}
        </div>
